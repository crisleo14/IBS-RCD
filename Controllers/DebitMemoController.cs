        ...
        public async Task<IActionResult> AasImport(IFormFile file, CancellationToken cancellationToken)
        {
            ...
            using (var stream = new MemoryStream())
            {
                ...
                try
                {
                    ...
                    var rowCount = worksheet.Dimension.Rows;
                    var dmDictionary = new Dictionary<string, bool>();
                    var debitMemoList = await _aasDbContext
                        .DebitMemos
                        .ToListAsync(cancellationToken);
                    ...
                    for (int row = 2; row <= rowCount; row++) // Assuming the first row is the header
                    {
                        ...
                        {
                            var dmChanges = new Dictionary<string, (string OriginalValue, string NewValue)>();
                            var existingDm = await _aasDbContext.DebitMemos.FirstOrDefaultAsync(si => si.OriginalDocumentId == debitMemo.OriginalDocumentId, cancellationToken);
                            var existingDmInLogs = await _dbContext.ImportExportLogs
                                .Where(x => x.DocumentNo == existingDm.DebitMemoNo)
                                .ToListAsync(cancellationToken);
                        ...
                        debitMemo.SalesInvoiceId = await _aasDbContext.SalesInvoices
                            .Where(c => c.OriginalDocumentId == debitMemo.OriginalSalesInvoiceId)
                            .Select(c => (int?)c.SalesInvoiceId)
                            .FirstOrDefaultAsync(cancellationToken);

                        debitMemo.ServiceInvoiceId = await _aasDbContext.ServiceInvoices
                            .Where(c => c.OriginalDocumentId == debitMemo.OriginalServiceInvoiceId)
                            .Select(c => (int?)c.ServiceInvoiceId)
                            .FirstOrDefaultAsync(cancellationToken);
                        ...
                        await _aasDbContext.DebitMemos.AddAsync(debitMemo, cancellationToken);
                    }
                    await _aasDbContext.SaveChangesAsync(cancellationToken);
                    await transaction.CommitAsync(cancellationToken);
